        ...
        */
        base64LoginResponse: function base64LoginResponse(requestInfo, referenceTagXPath, entity, user, rcallback, rtnCallback) {
            ...
            if(metadata && metadata.idp && metadata.sp) {
                ...
                resXml = SamlLib.constructSAMLSignature(rawSamlResponse, metadata.sp.isWantAssertionsSigned() ? referenceTagXPath : SamlLib.createXPath('Response'), metadata.idp.getX509Certificate('signing'), idpSetting.privateKeyFile, idpSetting.privateKeyFilePass, idpSetting.requestSignatureAlgorithm, false, {
                    prefix: 'ds',
                    location: {
                ...
                idpSetting.isAssertionEncrypted ? SamlLib.encryptAssertion(entity.idp, entity.sp, resXml, rtnCallback) : rtnCallback(Utility.base64Encode(resXml));
            } else {
                throw new Error('Missing declaration of metadata');
            }
        },
        ...
        base64LogoutResponse: function base64LogoutResponse(requestInfo, referenceTagXPath, entity, rcallback) {
            ...
            if(metadata && metadata.init && metadata.target) {
                ...
                if(initSetting.logoutResponseTemplate) {
                    ...
                    return SamlLib.constructSAMLSignature(rawSamlResponse, entity.target.entitySetting.wantLogoutResponseSigned ? referenceTagXPath : SamlLib.createXPath('Response'), metadata.idp.getX509Certificate('signing'), idpSetting.privateKeyFile, idpSetting.privateKeyFilePass, idpSetting.requestSignatureAlgorithm, false); // SS1.1 add signat ure algorithm
                }
            } else {
                throw new Error('Missing declaration of metadata');
            }
